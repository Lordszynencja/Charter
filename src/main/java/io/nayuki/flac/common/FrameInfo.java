...
/**
 * Represents most fields in a frame header, in decoded (not raw) form. Mutable
 * structure, not thread safe. Also has methods for parsing and serializing this
 * structure to/from bytes. All fields can be modified freely when no method
 * call is active.
 *
 * @see FrameDecoder
...
public final class FrameInfo {
	...
	/**
	 * The index of this frame, where the foremost frame has index 0 and each
	 * subsequent frame increments it. This is either a uint31 value or &minus;1 if
	 * unused. Exactly one of the fields frameIndex and sampleOffse is equal to
	 * &minus;1 (not both nor neither). This value can only be used if the stream
	 * info's minBlockSize = maxBlockSize (constant block size encoding style).
	 */
	public int frameIndex;

	/**
	 * The offset of the first sample in this frame with respect to the beginning of
	 * the audio stream. This is either a uint36 value or &minus;1 if unused.
	 * Exactly one of the fields frameIndex and sampleOffse is equal to &minus;1
	...
	/**
	 * The raw channel assignment value of this frame, which is a uint4 value. This
	 * indicates the number of channels, but also tells the stereo coding mode.
	 */
	public int channelAssignment;

	/**
	 * The number of samples per channel in this frame, in the range 1 to 65536
	 * inclusive.
	 */
	public int blockSize;

	/**
	 * The sample rate of this frame in hertz (Hz), in the range 1 to 655360
	 * inclusive, or &minus;1 if unavailable (i.e. the stream info should be
	 * consulted).
	 */
	public int sampleRate;

	/**
	 * The sample depth of this frame in bits, in the range 8 to 24 inclusive, or
	 * &minus;1 if unavailable (i.e. the stream info should be consulted).
	 */
	public int sampleDepth;

	/**
	 * The size of this frame in bytes, from the start of the sync sequence to the
	 * end of the trailing CRC-16 checksum. A valid value is at least 10, or
	 * &minus;1 if unavailable (e.g. the frame header was parsed but not the entire
	 * frame).
	 */
	...
	/**
	 * Constructs a blank frame metadata structure, setting all fields to unknown or
	 * invalid values.
	 */
	public FrameInfo() {
		frameIndex = -1;
	...
	/**
	 * Reads the next FLAC frame header from the specified input stream, either
	 * returning a new frame info object or {@code null}. The stream must be aligned
	 * to a byte boundary and start at a sync sequence. If EOF is immediately
	 * encountered before any bytes were read, then this returns {@code null}.
	 * <p>
	 * Otherwise this reads between 6 to 16 bytes from the stream &ndash; starting
	 * from the sync code, and ending after the CRC-8 value is read (but before
	 * reading any subframes). It tries to parse the frame header data. After the
	 * values are successfully decoded, a new frame info object is created, almost
	 * all fields are set to the parsed values, and it is returned. (This doesn't
	 * read to the end of the frame, so the frameSize field is set to -1.)
	 * </p>
	 *
	 ...
	 */
	public static FrameInfo readFrame(final FlacLowLevelInput in) throws IOException {
		// Preliminaries
		in.resetCrcs();
		...
		final int blockStrategy = in.readUint(1);
		...
		final long position = readUtf8Integer(in); // Reads 1 to 7 bytes
		if (blockStrategy == 0) {
			if ((position >>> 31) != 0) {
	...
	private static long readUtf8Integer(final FlacLowLevelInput in) throws IOException {
		final int head = in.readUint(8);
		final int n = Integer.numberOfLeadingZeros(~(head << 24)); // Number of leading 1s in the byte
		assert 0 <= n && n <= 8;
		if (n == 0) {
			return head;
		} else if (n == 1 || n == 8) {
	...
	private static int decodeBlockSize(final int code, final FlacLowLevelInput in) throws IOException {
		if ((code >>> 4) != 0) {
		...
		return switch (code) {
			...
			default -> {
				final int result = searchSecond(BLOCK_SIZE_CODES, code);
	...
	private static int decodeSampleRate(final int code, final FlacLowLevelInput in) throws IOException {
		if ((code >>> 4) != 0) {
			throw new IllegalArgumentException();
		}
		return switch (code) {
			...
			default -> {
				final int result = searchSecond(SAMPLE_RATE_CODES, code);
	...
	// Argument is a uint3 value. Pure function and performs no I/O. Return value is
	// in the range [-1, 24].
	private static int decodeSampleDepth(final int code) {
		if ((code >>> 3) != 0) {
			throw new IllegalArgumentException();
		} else if (code == 0) {
			return -1; // Caller should obtain value from stream info metadata block
		} else {
			final int result = searchSecond(SAMPLE_DEPTH_CODES, code);
			if (result == -1) {
	 ...
	 * This does not write the data of subframes, the bit padding, nor the CRC-16
	 * field.
	 * </p>
	 * <p>
	 * The stream must be byte-aligned before this method is called, and will be
	 * aligned upon returning (i.e. it writes a whole number of bytes). This method
	 * initially resets the stream's CRC computations, which is useful behavior for
	 * the caller because it will need to write the CRC-16 at the end of the frame.
	 * </p>
	 *
	 ...
	 * @throws IOException          if an I/O exception occurred
	 */
	public void writeHeader(final BitOutputStream out) throws IOException {
	...
	// Given a uint36 value, this writes 1 to 7 whole bytes to the given output
	// stream.
	private static void writeUtf8Integer(final long val, final BitOutputStream out) throws IOException {
		if ((val >>> 36) != 0) {
			throw new IllegalArgumentException();
		}
	...
	private static int getSampleDepthCode(final int sampleDepth) {
	...
	private static final int searchFirst(final int[][] table, final int key) {
		for (final int[] pair : table) {
			if (pair[0] == key) {
				return pair[1];
			}
		}
	...
	private static final int searchSecond(final int[][] table, final int key) {
		for (final int[] pair : table) {
			if (pair[1] == key) {
				return pair[0];
			}
