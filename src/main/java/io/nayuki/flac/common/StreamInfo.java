...
/**
 * Represents precisely all the fields of a stream info metadata block. Mutable
...
public final class StreamInfo {
	...
	/**
	 * Minimum block size (in samples per channel) among the whole stream, a uint16
	 * value. However when minBlockSize = maxBlockSize (constant block size encoding
	 * style), the final block is allowed to be smaller than minBlockSize.
	 */
	public int minBlockSize;

	/**
	 * Maximum block size (in samples per channel) among the whole stream, a uint16
	 * value.
	 */
	...
	/**
	 * The number of channels in the audio stream, between 1 and 8 inclusive. 1
	 * means mono, 2 means stereo, et cetera.
	 */
	 ...
	 * The special value of 0 signifies that the value is unknown (not empty
	 * zero-length stream).
	 */
	public long numSamples;

	/**
	 * The 16-byte MD5 hash of the raw uncompressed audio data serialized in little
	 * endian with channel interleaving (not planar). It can be all zeros to signify
	 * that the hash was not computed. It is okay to replace this array as needed
	 * (the initially constructed array object is not special).
	 */
	public byte[] md5Hash;
	...
	/**
	 * Constructs a stream info structure by parsing the specified 34-byte metadata
	 * block. (The array must contain only the metadata payload, without the type or
	 * length fields.)
	 *
	 ...
	 */
	public StreamInfo(final byte[] b) {
		Objects.requireNonNull(b);
		if (b.length != 34) {
			throw new IllegalArgumentException("Invalid data length");
		}
		try (FlacLowLevelInput in = new ByteArrayFlacInput(b)) {
	...
	/**
	 * Checks the state of this object, and either returns silently or throws an
	 * exception.
	 *
	 ...
	 */
	public void checkFrame(final FrameInfo meta) {
	...
	/**
	 * Writes this stream info metadata block to the specified output stream,
	 * including the metadata block header, writing exactly 38 bytes. (This is
	 * unlike the constructor, which takes an array without the type and length
	 * fields.) The output stream must initially be aligned to a byte boundary, and
	 * will finish at a byte boundary.
	 *
	 ...
	 */
	public void write(final boolean last, final BitOutputStream out) throws IOException {
	...
	/**
	 * Computes and returns the MD5 hash of the specified raw audio sample data at
	 * the specified bit depth. Currently, the bit depth must be a multiple of 8,
	 * between 8 and 32 inclusive. The returned array is a new object of length 16.
	 *
	 * @param samples the audio samples to hash, where each subarray is a channel
	 *                (all not {@code null})
	 * @param depth   the bit depth of the audio samples (i.e. each sample value is
	 *                a signed 'depth'-bit integer)
	 ...
	 */
	public static byte[] getMd5Hash(final int[][] samples, final int depth) {
		// Check arguments
