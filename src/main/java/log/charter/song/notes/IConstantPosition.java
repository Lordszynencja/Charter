...
public interface IConstantPosition extends Comparable<IConstantPosition> {
	...
	public static <T extends IConstantPosition> T findClosest(final ArrayList2<T> positions, final int position) {
	...
	public static <T extends IConstantPosition> T findLastBeforeEquals(final List<T> list, final int position) {
		final int id = findLastIdBeforeEqual(list, position);
		return id < 0 ? null : list.get(id);
	}

	public static <T extends IConstantPosition> T findLastBefore(final List<T> list, final int position) {
		final int id = findLastIdBefore(list, position);
		return id < 0 ? null : list.get(id);
	}

	public static <T extends IConstantPosition> List<T> getFromTo(final ArrayList2<T> list, final int from,
			final int to) {
	...
	@Override
	default int compareTo(final IConstantPosition o) {
