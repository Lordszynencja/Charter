...
public class LowPassSettings extends RowedDialog {
	...
	public LowPassSettings(final CharterFrame charterFrame) {
		super(charterFrame, Label.LOW_PASS_SETTINGS, inputSize * 12);
	...
	private FieldWithLabel<TextInputWithValidation> addFrequencyInput(final RowedPosition position) {
		final TextInputWithValidation input = TextInputWithValidation.generateForDouble(frequency, inputSize * 2,
	...
	private FieldWithLabel<TextInputWithValidation> addRippleDbInput(final RowedPosition position) {
		...
		final FieldWithLabel<TextInputWithValidation> field = new FieldWithLabel<>(Label.PASS_FILTER_RIPPLE_DB,
				inputSize * 3, inputSize * 2, inputSize, input, LabelPosition.LEFT);
		panel.add(field, position);
		position.newRow();
